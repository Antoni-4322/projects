...
switch ($ITEM)
{
    ...
    "TotalRunningJobs" {
        [System.DateTime]$ExcludeDate = (Get-Date).adddays(-30)
        $query = (Get-VBRBackupSession | where {$_.State -ccontains "Working"} | Where-Object {$_.CreationTime -ge $ExcludeDate}).count
    ...
    "TotalJobsSuccess" {
        [System.DateTime]$ExcludeDate = (Get-Date).adddays(-1)
        write-output (Get-VBRBackupSession | where {$_.Result -ccontains "Success"} | Where-Object {$_.CreationTime -ge $ExcludeDate}).count
		
	}

    "TotalJobsFailed" {
        [System.DateTime]$ExcludeDate = (Get-Date).adddays(-1)
        write-output (Get-VBRBackupSession | where {$_.Result -ccontains "Failed"} | Where-Object {$_.CreationTime -ge $ExcludeDate}).count
		
	}

    "TotalJobsWarning" {
        [System.DateTime]$ExcludeDate = (Get-Date).adddays(1)
        write-output (Get-VBRBackupSession | where {$_.Result -ccontains "Warning"} | Where-Object {$_.CreationTime -ge $ExcludeDate}).count
    ...
    "RepoTotal" {
		$result = (Get-VBRBackupRepository).count
        [string]$result
	}
    ...
    "RepoTotalPercentage" {
		    $query = Get-CimInstance -Class Repository -ComputerName $env:COMPUTERNAME -Namespace ROOT\VeeamBS | Where-Object { $_.Name -eq "$ID" }
		    $num1 = $query | Select-Object -ExpandProperty FreeSpace
            $num2 = $query | Select-Object -ExpandProperty Capacity
